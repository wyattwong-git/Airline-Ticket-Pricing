...
### **Business Challenges**
The core challenge was to identify why airline fare prices varies so significantly across similar routes and how city-level market power influences these prices. Key questions addressed include:
...
- **Modeling and prediction:** Can route-level fares be predicted using distance, demand, competition, and endpoint hub characteristics? Which features contribute most to explaining fare variation across markets?
### **Methodology**
We employed a phased Ordinary Least Squares (OLS) Regression approach to isolate the impact of different forces:
- **Baseline Modeling (Phase 1):** Regressed log-transformed fare against log-distance and log-passengers to account for "economies of scale" in longer flights and high-density routes.
...
- **The Equalizing Force:** LCC Volume is the most consistent check against high prices. It is not just the presence of a budget airline that matters, but the volume of seats they offer. High LCC volume forces even dominant hub carriers to lower fares to remain competitive.
